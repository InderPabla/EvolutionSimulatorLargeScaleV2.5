...
public class CustomCircleCollider
{
    ...
    {
        float dx, dy, A, B, C, det, t, indx, indy, cx, cy, inDistance1, inDistance2, distance, inDisShort;
        ...
        else if (det == 0)
        {
            ...
            inDistance1 = Vector2.Distance(intersection1, point1);
            distance = Vector2.Distance(point2, point1);

            if (Mathf.Sign(dx) == Mathf.Sign(indx) && Mathf.Sign(dy) == Mathf.Sign(indy) && inDistance1<=distance)
                return inDistance1;
            else
                return -1f;
        }
        else
        {
            ...
            indy = intersection1.y - point1.y;
            inDistance1 = Vector2.Distance(intersection1, point1);
            inDistance2 = Vector2.Distance(intersection2, point1);
            distance = Vector2.Distance(point2, point1);

            inDisShort = inDistance1 < inDistance2 ? inDistance1 : inDistance2;

            if (Mathf.Sign(dx) == Mathf.Sign(indx) && Mathf.Sign(dy) == Mathf.Sign(indy) && inDisShort <= distance)
                return inDisShort;
            else
                return -1f;
        }
